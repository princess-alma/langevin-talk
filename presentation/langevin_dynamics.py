...
class LangevinDynamics(Scene):
    def construct(self):
        ...
        # Create the main blue ball (same size as original)
        large_ball = Circle(radius=large_ball_radius, color=BLUE, fill_opacity=1)
        large_ball.move_to(LEFT * 2 + DOWN * 1)  # Same starting position
        self.add(large_ball)

        # Create many smaller balls (thermal environment)
        small_balls = []
        for i in range(num_small_balls):
            ...
            while True:
                ...
                if np.linalg.norm(pos - large_ball.get_center()) > large_ball_radius + small_ball_radius + 0.15:
        ...
        large_velocity = np.array([3.0, 2.0])  # Same as original
        small_velocities = []
        ...
        def handle_wall_collision(pos, vel, radius):
            """Handle wall collisions and return corrected position and velocity"""
            new_pos = pos.copy()
            new_vel = vel.copy()
            
            # Left and right walls
            if pos[0] <= -box_size + radius:
                new_vel[0] = abs(new_vel[0])
                new_pos[0] = -box_size + radius
            elif pos[0] >= box_size - radius:
                new_vel[0] = -abs(new_vel[0])
                new_pos[0] = box_size - radius
            
            # Top and bottom walls
            if pos[1] <= -box_size + radius:
                new_vel[1] = abs(new_vel[1])
                new_pos[1] = -box_size + radius
            elif pos[1] >= box_size - radius:
                new_vel[1] = -abs(new_vel[1])
                new_pos[1] = box_size - radius
        ...
        def update_large_ball(mob, dt):
            nonlocal large_velocity
            ...
            for i, small_ball in enumerate(small_balls):
                ...
                large_velocity, small_velocities[i] = check_ball_collision(
                    current_pos, large_velocity, large_ball_radius,
                    small_pos, small_velocities[i], small_ball_radius,
                    mass1=15.0, mass2=1.0  # Increased mass from 10.0 to 25.0
                )
            
            # Update position
            new_pos = current_pos + large_velocity * dt
            new_pos, large_velocity = handle_wall_collision(new_pos, large_velocity, large_ball_radius)
        ...
        def create_small_ball_updater(ball_index):
            def update_small_ball(mob, dt):
                ...
                thermal_noise = np.random.normal(0, 0.4, 2)  # Slightly reduced noise
                ...
                for j in range(max(0, ball_index - 2), min(len(small_balls), ball_index + 3)):  # Reduced range for performance
                    if j != ball_index:
                ...
                new_pos, small_velocities[ball_index] = handle_wall_collision(
                    new_pos, small_velocities[ball_index], small_ball_radius
                )
        ...
        # Add updaters
        large_ball.add_updater(update_large_ball)
